...
class Chain:
    def __init__(self, genesisBlock: Block, name):
        ...
        for output in genesisBlock.tx.output:
            key = generate_hash([genesisBlock.tx.number.encode(
                "utf-8"), output['pubkey'].encode("utf-8")])
    ...
    def addTx(self, tx: Transaction):
        ...
        self.unspentCoin2BlockIdx = Chain.validateBlock(
            self.blocks, newBlock, self.unspentCoin2BlockIdx)
    ...
    def validateBlock(blocks, newBlock: Block, unspentCoin2BlockIdx):
        ...
        for nextTxInput in newTx.input:
            ...
            key = generate_hash(
                [newTxInputNum.encode('utf-8'), senderPk.encode('utf-8')])

            # i am claiming to use this coin
            if(key not in unspentCoin2BlockIdx):
                raise Exception(
                    "Either a double spend or new block is spending money that was never made")
            ...
            if(not foundOutput):
                raise Exception(
                    "user " + nextTxInput['pubkey'] + " does not have money they are claiming")
        ...
        for output in newBlock.tx.output:
            ...
            newKey = generate_hash(
                [newTx.number.encode("utf-8"), recieverKey.encode("utf-8")])
        ...
        try:
            ...
            for block in blocks[1:]:
                if block.pow < 0x07FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF:
                    unspentCoin2BlockIdx = Chain.validateBlock(
                        builtBlocks, block, unspentCoin2BlockIdx)
                    if(not unspentCoin2BlockIdx):
                        raise Exception(
                            "Incoming chain was found to have invalid block")
                    else:
